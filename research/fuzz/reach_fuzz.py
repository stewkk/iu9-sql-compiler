...
def _make_reachability_data_dir(source_dir: Path, schema: "Schema", tmp_dir: Path) -> Path:
    ...
    with (tmp_dir / "indexes.meta").open("w") as meta:
        meta.write("# Generated by reach_fuzz.py for optimizer reachability checks.\n")
        for table in sorted(schema.tables.values(), key=lambda t: t.name):
            for col in table.columns:
                if col.type == "int":
                    meta.write(
                        f"{table.name} {col.name} sorted {table.name}.{col.name}.sorted.idx\n"
                    )
    return tmp_dir
...
def main() -> None:
    ...
    reach_tmp = tempfile.TemporaryDirectory(prefix="iu9-sql-reach-")
    reach_data_dir = _make_reachability_data_dir(
        source_data_dir, schema, Path(reach_tmp.name)
    )
