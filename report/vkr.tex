...
Модельная СУБД для того же запроса строит другой достижимый план, представленный
на рисунке~\ref{fig:reach-fuzz-cross-ours}. В нем фильтр по таблице
\verb|markets| используется как предикат внутреннего соединения, а не как
отдельный оператор \verb|PhysicalFilter| над чтением этой таблицы.
...
запросов на том же наборе данных. Видно, что оптимизатор дает ускорение на
$20-50\%$, а на накоторых запросах в $6$ и более раз.
