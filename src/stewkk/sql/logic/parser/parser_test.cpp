...
TEST(ParserTest, SelectSingleColumnFromSingleTable) {
    ...
    ASSERT_THAT(got,
                VariantWith<Projection>(Projection{std::vector<Attribute>{"users.id"},
...
TEST(ParserTest, SelectMultipleColumnsFromSingleTable) {
    ...
    ASSERT_THAT(got,
                VariantWith<Projection>(Projection{std::vector<Attribute>{"users.id", "users.email", "users.phone"},
                                                   std::make_shared<Operator>(Table{"users"})}));
}
